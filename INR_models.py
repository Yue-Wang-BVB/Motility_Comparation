...
class ImplicitRegistrator:
    ...
    # ──────────────────────────────────────────────
    # 2-D displacement field prediction helper
    # ──────────────────────────────────────────────
    def predict_field(self, outshape, scale_to_pixel=True):
        """
        Quick helper for 2-D use-case.
        Args
        ----
          outshape : (H,W) 目标平面大小
          scale_to_pixel : 若为 True，则把网络输出从 [-1,1] 归一化空间
                           转回到像素单位 (dx,dy)；保持与光流方向一致
        Returns
        -------
          field_np : ndarray, shape = (H,W,2), dtype=float32
        """
        H, W = outshape
        device = self.moving_image.device
        # 固定 z=0，沿 (y,x) 网格采样
        ys = torch.linspace(-1, 1, H, device=device)
        xs = torch.linspace(-1, 1, W, device=device)
        yy, xx = torch.meshgrid(ys, xs, indexing='ij')
        coords = torch.stack([yy, xx, torch.zeros_like(xx)], dim=-1)    # (H,W,3)
        coords_flat = coords.reshape(-1, 3).requires_grad_(False)

        with torch.no_grad():
            dvec = self.network(coords_flat)         # (N,3)

        dvec = dvec.reshape(H, W, 3)                 # 仍是 (H,W,3)
        field_np = dvec[..., :2].cpu().numpy()       # 取 xy 两维

        if scale_to_pixel:
            field_np[..., 0] *= 0.5 * (H - 1)
            field_np[..., 1] *= 0.5 * (W - 1)

        return field_np.astype(np.float32)
